...
class WXR_Importer extends WP_Importer {
	...
	protected function import_end() {
        ...
        $data_import_optionx         = get_option('goso_import_demo_data');
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		$only_import_pages = $this->options['only_import_pages'];
		$post_type = isset( $data['post_type'] ) ? $data['post_type'] : '';

		if ( $only_import_pages && 'page' != $post_type ) {
			return false;
		}
        ...
        $post__type = isset($data['post_type']) ? $data['post_type'] : '';

        if (!isset($data_import_option['post__type'])) {
            $data_import_option['post__type'] = array();
        }

        $data_import_option['post__type'][$post__type][$post_id] = '';
        update_option( Goso_Authow_Demo_Importer::$data_import_option, $data_import_option );
